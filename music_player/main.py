...
class PlayingPage(BoxLayout):
    def __init__(self):
        ...
        self.toolbar = MDTopAppBar(title="Now Playing")
...
class SettingsPage(BoxLayout):
    def __init__(self):
        super().__init__()
        self.orientation = "vertical"
        self.toolbar = MDTopAppBar(title="Setting")
        self.toolbar.left_action_items = [["arrow-left", self.back]]
        self.add_widget(self.toolbar)
...
class ListPage(BoxLayout):
    def __init__(self):
        ...
        self.orientation = "vertical"
        self.toolbar = MDTopAppBar(title="Music Player")
        ...
        try:
            for music in self.music_list:
                ...
                self.dl = TwoLineListItem(
                    text=(meta["title"] or path),
                    secondary_text=(meta["artist"] or "Unkown"),
                )
    ...
    for root, dirnames, filenames in os.walk(path):
        for nm in filenames:
            ...
            if os.path.splitext(nm)[1] == ".mp3":
                music_list.append(
                    {
                        "path": str(os.path.join(root, nm)),
                        "meta": get_meta(str(os.path.join(root, nm))),
                    }
                )
    return music_list
